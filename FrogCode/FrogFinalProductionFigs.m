...
%calculate 95% CI for temeprature
    ...
    xneg(temp,:) = Temps(obsTemps(temp))-(Temps(obsTemps(temp))+errTemp);
    xpos = xneg;
    %catch and determine min/max replicate size
    if temp == 1
        rep(1) = 1;
        rep(2) = max(N);
    else
...
for stage = 1:length(lnPseudoKperStage{1})
    ...
    %plot fit lines
        ...
        text(1./(273.15+27.5),-6.25,strcat("E_a = ",num2str(round((-lineFit.p1*R)...
            *tokJConvert,0))," kJ/mol"),'Color','k','FontSize',23)
...
xlabel("Frog intervals",'FontWeight','Bold','FontSize',38)
yticks([0:1/7:1])
yticklabels([40:10:110])
ylabel("Apparent activation energy (kJ/mol)",'FontWeight','Bold','FontSize',38)
legend([black red],["Median E_a","68% CI in E_a"],'Location','northwest','LineWidth',2.5,'FontSize',42)
%title(["Distribution of Bootstrapped Frog Interval Activation Energies"],'FontSize',30)
...
%%
%Figure 3C, for viable, not linear range, so exclude only nonviable
...
%remove T0 time point from analysis
    ...
    stageAbbsNoT0 = stageAbbs(obsScores);  
    stageNamesNoT0 = stageNames(obsScores);
    excludedTemps = [61];%[1 sizeData-1 sizeData];%
        %excluded temps = only nonviable data for this plot
    viableTemps = [2:sizeData-2];
            %for full range [61];
            %for "linear range"
...
for stage = justThree
   ...
   %iterate over temperaters to average embryos for each temp then plot
    ...
    %beautify
        ...
        %gca, titles, legend
            text(1./(273.15+19.75),theoryMax-0.25,strcat(...
...
%pvals
    ...
    %h1.Title = {'\fontsize{38}Frog ANCOVA p-values',...
        %'\fontsize{38}comparing individual intervals'};
    h1.Title = {'\fontsize{38}Frog ANCOVA p-values'};
    h1.FontSize = 15;
...
%Non-scaled Aestetics    
    ...
    xtickangle(45)
    set(gca,'linewidth', 3, 'FontSize', 28)
    %title({"Apparent activation energy", " Frog developmental intervals"}, "FontSize", 28)
    ylabel("Apparent activation energy (kJ/mol)", "FontSize", 28,'FontWeight','Bold')
    xlabel("Frog interval", "FontSize", 28,'FontWeight','Bold')
    ylim([40 120]);
...
for i = 1:1
    ...
    %Horizontal line connecting above two lines
        ...
        text(mean([nonScaled(largerTime) nonScaled(smallerTime)]),...
            smallerEa-6,'\fontsize{20}#','Color','m','HorizontalAlignment','center');
end

%Draw Braces between significantly different Eas, Use ancova calc above
%DRAWBRACE([X1,Y1], [X2,Y2], W)
%rightside up
    y1 = stageActEa(11)+spos(11)+1;
    drawbrace([4,y1], [11,y1], 10,'LineWidth',2.5,'Color','k');
        text(mean([11 4]),y1+4,'\fontsize{25}***','HorizontalAlignment','center');
%%
%Response to Reviewers
%Comparing Eas with statistical significance between stage-wise
%Must run the above stages code chunk first
...
%Scaled Aestetics    
    ...
    %labels
        ...
        %title([{"Apparent activation energie: Scaled by proportion of frog development"}],...
            %"FontSize", 25)

        ylabel("Apparent activation energy (kJ/mol)", "FontSize", 23,'FontWeight','Bold')
        title(strcat("Frog interval activation energies (Scaled by developmental interval times at ",...
...
for i = 1:1
    ...
    %line from larger Ea up to horizontal, allow 1 unit space between error
    %bar and line
        line(repmat(meanXs(largerTime),2,1), [largerEa-1 largerEa-2],...
            'LineWidth',2.5,'Color','m');
    %line from smaller Ea up to horizontal,allow 1 unit space between error
    %bar and line
        line(repmat(meanXs(smallerTime),2,1), [smallerEa-1 largerEa-2],...
            'LineWidth',2.5,'Color','m');
    %Horizontal line connecting above two lines
        line([meanXs(largerTime) meanXs(smallerTime)], repmat(...
            largerEa-2,2,1),'LineWidth',2.5,'Color','m');
        text(mean([meanXs(largerTime) meanXs(smallerTime)]),...
            smallerEa-6,'\fontsize{20}#','Color','m','HorizontalAlignment','center');
end
...
%rightside up
    y1 = stageActEa(11)+spos(11)+1;
    drawbrace([meanXs(4),y1], [meanXs(11),y1], 10,'LineWidth',2.5,'Color','k');
        text(mean([meanXs(4) meanXs(11)]),y1+6,'\fontsize{25}***','HorizontalAlignment','center');
...
for p = 1:2
    if p == 1
        excludedTemps = [1 length(invertedTemps)-1 length(invertedTemps)];
            %here excluded temps should only be non-viable temperatures, as
            %we are looking at BIC over the viable range only
    else
        excludedTemps = [61];
    end
	...
	%For every temperature build up the fitting data
        ...
        if startStage~=endStage
            ...
            if length(preDec{1}) >= 2
                BICMat(endStage,startStage) = round(log(BICComps),4);
            else
                BICMat(endStage,startStage) = round(log(BICComps),4);
            end
...
%plot the relevent BIC data range and the associated stage abbreviations
    ...
    xlabels = stageAbbsNoT0(1:11);
        ...
        h = heatmap(xlabels, ylabels, round(real(BICMat(2:12,1:11)),2,'significant'),'CellLabelColor','none');
...
%Aestetics
    ...
    if p == 1
        title('\fontsize{38}Frog ln(L_{Q}/L_{L}): viable temp range');
    else
...
%
    for x = 1:numScores-1
        for y = x:numScores-1
            ...
            if p == 1
                text(xShift + scaleX*x,yShift - scaleY*y,txt,'Color','k','FontSize',h.FontSize-8,'HorizontalAlignment','center','VerticalAlignment','middle')
            else
...
%calculate and plot silico prediction first
    ...
    %parse constants and data from stage figure
        rangeT = [Temps(2) Temps(length(Temps)-5)];
    ...
    %Eas
        text(1./(273.15+17.35),-10.35,strcat("E_a = ",num2str(round((-mdl2(1)*R)*tokJConvert,0))," kJ/mol"),'Color','k','FontSize',24)
        text(1./(273.15+17.35),-10.60,strcat("E_a = ",num2str(round((-mdl(1)*R)*tokJConvert,0))," kJ/mol"),'Color','c','FontSize',24)
    ...
    %seperate to avoid being affected by gca
        ...
        legend([lm lF],{'Biological Fit: 12.2-25.7^oC','Predictive Fit'},'FontSize',24)
