...
%Destination
    ...
    folder = '/Users/josephcrapse/Desktop/GAPDH/';
...
%calculate 95% CI for temeprature
    ...
    legend([blue, linfit, qfit],{'0.47mM GAP, 4.7mM NAD+',...
...
%control comparison
    ...
    xlim([-7 -3])
    ylim([-7 -3])
    xlabel('ln(k) at 0.24mM GAP, 2.4mM NAD+')
    ylabel('ln(k) at 0.47mM GAP, 4.7mM NAD+')
    title('GAPDH Kinetics Control')
    for i = 1:length(temps)
...
%non-controls   
    ...
    folder = '/Users/josephcrapse/Desktop/BGal/';  
...
%plot k and kCont against T    
    ...
    ylabel('ln(k) (rate)')
    title('Updated: Enzyme Kinetics of B-Gal')
    set(gca,'linewidth', 2.5, 'FontSize', 20);
%
%plot with errorbars for exp  
figure('Position',[100 100 600 500])
...
%control plot
    ...
    hold on
    for i = 1:length(tenMM)
        dataPoints(i) = scatter(tenMM(i),twentyMM(i),[],dColors(i,:),'LineWidth',2);
    end
    xlim([-5 0])
    ylim([-5 0])
    xlabel('ln(k) at 10 mM')
    ylabel('ln(k) at 20 mM')
    title('B-Gal Kinetics: 10 mM vs 20 mM')
    counter = 1;
    for i = [1 3 6 8]
