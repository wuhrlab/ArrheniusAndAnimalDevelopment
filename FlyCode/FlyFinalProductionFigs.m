...
for stage = [1,3:length(lnPseudoKperStage{1})]
    ...
    %plot fit lines
        ...
        text(1./(273.15+23.5),-5.25,strcat("E_a = ",num2str(round(...
            (-lineFit.p1*R)*tokJConvert,0))," kJ/mol"),'Color','b',...
            'FontSize',19); 
...
legend([black red],["Median E_a","68% CI in E_a"],'Location','northeast',...
    'LineWidth',2.5,'FontSize',20)
set(gca,'lineWidth',2.5,'FontSize',20)
title(["Distribution of Bootstrapped Activation Energies";...
    "Fly Developmental Stages"],'FontSize',25)
...
for stage = [5,6]
    ...
    %plot fit lines
        ...
        text(1./(273.15+19.5),-5.55,strcat("E_a = ",num2str(round(...
            (-lineFit.p1*R)*tokJConvert,0))," +/- ", num2str(round(...
            (plusCI*R)*tokJConvert,0))),'Color','b','FontSize',24)
        text(1./(273.15+14.5),-6.0,"kJ/mol",'Color','b','FontSize',24)
   ...
   %iterate over temperaters to average embryos for each temp then plot
    ...
    %beautify
        %ylabels
            ...
            yticklabels(ticks);
            text(1./(273.15+17.5),theoryMax-0.35,strcat(...
...
%pvals
    ...
    h1.Title = {'\fontsize{40}Fly ANCOVA p-values',...
        '\fontsize{40}comparing individual stages'};
    h1.FontSize = 15;
...
figure('Position',[100 100 1100 900]);        
    ...
    h1.Title = {'\fontsize{40}Fly ANCOVA power analysis', '\fontsize{40}comparing individual stages'};
    h1.FontSize = 15;
    h1.MissingDataColor = [1 1 1];
    h1.GridVisible = 'off';
    caxis([0 1])
...
%Non Scaled Aestetics   
    ...
    set(gca,'linewidth', 3, 'FontSize', 28)
    title([{"Apparent activation energy", " Fly developmental stages"}],...
        "FontSize", 28)
...
%Scaled Aestetics    
    %developmentally scaled ticks
        xlim([min(tempScale)-2*abs(min(tempScale(3))) max(tempScale)+2*...
            abs(min(tempScale(3)))])
    ...
    %tick aestethics
        ...
        xticklabels(labs)
        xtickangle(45)
        text(tempScale(4), 38, innerLabs(1),'Rotation',45,'HorizontalAlignment','left','FontSize',28)
        text(tempScale(6)+800, 38, innerLabs(2),'Rotation',45,'HorizontalAlignment','left','FontSize',28)
...
%upside down
	%between 8 an 9
    	y1 = stageActEa(6)-spos(6)-1;
    	drawbrace([tempScale(7),y1], [tempScale(6),y1], 10,'LineWidth',2.5,'Color','k')
         	text(mean([tempScale(7) tempScale(6)])-1100,y1-5.25,'\fontsize{25}***');
...
%plot the relevent BIC data range and the associated stage abbreviations
    ...
    %exp space
        h = heatmap(xlabels, ylabels, round(BICMat(2:12,1:11),2),'CellLabelColor','black');
...
%Aestetics
    ...
    if p == 1
        title('\fontsize{40}Fly ln(L_{Q}/L_{L}): Linear Range');
    else
...
%calculate and plot silico prediction first
    ...
    %Eas
        text(1./(273.15+16.5),-10.25,strcat("E_a = ",num2str(round(...
            (-mdl2(1)*R)*tokJConvert,0))," kJ/mol"),'Color','b','FontSize',24)
        text(1./(273.15+16.5),-10.55,strcat("E_a = ",num2str(round(...
...
%calculate and plot silico prediction first
    ...
    %calculate and plot biological linear regression
        ...
        ylim([mean(allMeans)-2 max(yLin)])
        %ylim([mean(allMeans)-2 mean(allMeans)+1.25])
        
        lowerYCorrection = (min(yLin) - (mean(allMeans)-2))/(max(yLin)-min(yLin));
    ...
    %worst case
        ...
        %aesthetics
            ...
            %convert decimal xticks to 1/T in K
                set(gca,'linewidth', 3, 'FontSize', 28);
                legend([blue magenta black], {'Biological Data: A - G','Worst Case Model',strcat("Linear Fit ",num2str(Temps(2)-273.15),"-", num2str(Temps(length(Temps)-2)-273.15)," ^oC")},'FontSize',24);    
%%
