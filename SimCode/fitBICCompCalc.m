function BICcomp = fitBICCompCalc(data,mdl,mdl2,stage)
    ...
    %AICcomp = exp((2*(k-k2) - n*log(SSR2/SSR) + (c1-c2))/2);%AIC with
        %correction
    BICcomp = exp((log(n)*(k-k2) - n*log(SSR2/SSR))/2); %BIC drastically improve, 
        %but still seems very off, cuts every roughly in half
        %the quantity exp((AICmin???AICi)/2) can be interpreted as being 
        %proportional to the probability that the ith model minimizes the (estimated) information loss
end
